...
describe("POST users/:username/jobs/:id/:state?", () => {
  test("works", async () => {
    const resp = await request(app)
        .post(`/users/u3/jobs/${jobIds[2]}`)
        .set("authorization", `Bearer ${adminToken}`);
...
describe("PATCH users/:username/jobs/:id/:state?", () => {
  test("works", async () => {
    const resp = await request(app)
        .patch(`/users/u1/jobs/${jobIds[0]}/interested`)
        .set("authorization", `Bearer ${adminToken}`);
    expect(resp.statusCode).toEqual(200);
    expect(resp.body).toEqual({
      applied: {
